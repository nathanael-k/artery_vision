...
arteryGraph &ArteryReplicator::build_graph() {
  ...
  // tail recursive!
  void ArteryReplicator::explore_node(
      arteryNode & node, const arteryNode &old_node,
                    std::list<arteryNode *> hot_ends, 
      std::list<arteryNode *> new_ends, std::list<arteryNode *> hot_junctions,
      const size_t frame) {
    ...
    if (max_connections == 1) {
      auto it = find_ball_at(end_balls, ball.center_m);
      if (it != end_balls.end()) {
        ball = **it;
        end_balls.erase(it);
      ...
      next_node = curr_node->addNode(ball);
    ...
    if (max_connections == 2) {
      ...
      pre_ball = ball;
      ball = triangulate_ball(next_circle_A, next_circle_B, camera);
    }
    ...
    if (max_connections == 3) {
      if (!junction_balls.empty()) {
        auto it = find_ball_at(junction_balls, ball.center_m);
        if (it != end_balls.end()) {
          next_ball = **it;
          junction_balls.erase(it);

          std::cout << "Ending line at pre registered junction ..."
                    << std::endl;
        } else {
